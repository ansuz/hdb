 ...
 */
    ...
    remember.array=function(key,newArray,each,done){
        remember.generic.update(key,newArray,"ARRAY",function(Old,New,save){
            try{
                Old=JSON.parse(Old);
                New.forEach(function(v){
                    addIfAbsent(Old,v,each);
                });
                save(JSON.stringify(Old));
                if(done){
                    done(Old);
                }
            }catch(err){
                if(remember.log.parse){
                    console.error("[ARRAY PARSE ERROR]: %s",err);
                }
            }
        });
    };
    ...
    remember.ips=function(ips,done){
        remember.array('knownNodes',ips,hooks.newNode,done);
    };
